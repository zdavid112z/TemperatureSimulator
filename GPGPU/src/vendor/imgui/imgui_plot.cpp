...
namespace ImGui {
		...
		ImVec2 graph_size)
	{
		...
		if (ItemHoverable(inner_bb, id) && inner_bb.Contains(g.IO.MousePos))
		{
			...
			float y = ImClamp((g.IO.MousePos.y - inner_bb.Min.y) / (inner_bb.Max.y - inner_bb.Min.y), 0.0f, 0.9999f);
			y = (1.f - y) * (scale_max - scale_min) + scale_min;
